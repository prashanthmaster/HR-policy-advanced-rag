...
| T-5.3 | RAGAS: Context Precision, Context Recall, Faithfulness, Answer Correctness | T-5.2 | `DONE` (first real numbers: Precision 0.726, Recall 0.492, Faithfulness 0.089, Answer Correctness 0.169, over 11/24 ANSWERED items -- root cause found in Session 8 (narrative-substance defect, now fixed); those numbers are pre-fix and need a real re-run, see Phase 5 detail) |
| T-5.4 | Custom Citation Accuracy (LLM-as-judge, FinGuard G-Eval pattern) | T-5.3 | `WIP` (0.214 over 11/24 ANSWERED items was the post-retrieval-fix number; superseded pending re-run against the Session 8 narrative-substance fix -- see Phase 5 detail) |
| T-5.5 | Four-class confusion matrix + **over-refusal counter as a first-class defect** (Finding 3) | T-5.2 | `WIP` (`eval/run_confusion_matrix.py` written and smoke-tested; not yet run for real -- should run against the Session 8 narrative-substance fix, not the earlier pre-fix generator) |
