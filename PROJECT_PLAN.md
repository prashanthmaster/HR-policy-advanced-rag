...
| **M5** | Baseline scored | All 5 metrics + the four-class confusion matrix run on the golden set; **first real numbers** recorded | D4–D5 | `WIP` (T-5.3 RAGAS real first numbers in: Precision 0.726, Recall 0.492, Faithfulness 0.089, Answer Correctness 0.169 -- low Faithfulness/Answer Correctness flagged, not yet root-caused; T-5.4 Citation Accuracy real post-fix number in: 0.214; T-5.5/T-5.6 not started) |
...
| T-5.3 | RAGAS: Context Precision, Context Recall, Faithfulness, Answer Correctness | T-5.2 | `DONE` (real first numbers: Precision 0.726, Recall 0.492, Faithfulness 0.089, Answer Correctness 0.169, over 11/24 ANSWERED items -- Faithfulness/Answer Correctness low and not yet root-caused, see Results Ledger honest-read paragraph) |
...
| T-5.5 | Four-class confusion matrix + **over-refusal counter as a first-class defect** (Finding 3) | T-5.2 | `WIP` (`eval/run_confusion_matrix.py` written and smoke-tested; not yet run for real with the reranker -- see the two real diagnoses below first) |
...
**Scope call needed before this is fixed, not made unilaterally here**: this is a real, newly-found Phase 4 defect (`grading/temporal_reasoner.py`/`generation/generator.py`, both currently `DONE`), distinct from Phase 3's already-fixed retrieval bug and from the documented P-01/P-17 reranker limitation. Per Convention 15's pattern (present real data plainly, offer scoped options, let Prashanth make the scope call before reopening an already-DONE phase), this is flagged here rather than patched. T-5.5's confusion matrix and T-5.6's baseline entries proceed regardless per Prashanth's explicit instruction -- they record what is honestly true right now, defect included -- but the Results Ledger's Faithfulness/Answer-Correctness/Citation-Accuracy rows should carry this root cause inline, not the earlier unconfirmed arithmetic-vs-literal-text hypothesis, which real data does not support.
...
| 2026-09-04 | T-5.3 | RAGAS Context Precision (mean, 11/24 items reaching ANSWERED), LLM-judged (gpt-4o-mini) | **0.726** | `build/ragas_eval_result.json`, `eval/run_ragas_eval.py` |
| 2026-09-04 | T-5.3 | RAGAS Context Recall (mean, 11/24 items reaching ANSWERED), LLM-judged | **0.492** | `build/ragas_eval_result.json`, `eval/run_ragas_eval.py` |
| 2026-09-04 | T-5.3 | RAGAS Faithfulness (mean, 11/24 items reaching ANSWERED), LLM-judged | **0.089** | `build/ragas_eval_result.json`, `eval/run_ragas_eval.py` |
| 2026-09-04 | T-5.3 | RAGAS Answer Correctness (mean, 11/24 items reaching ANSWERED), LLM-judged | **0.169** | `build/ragas_eval_result.json`, `eval/run_ragas_eval.py` |
