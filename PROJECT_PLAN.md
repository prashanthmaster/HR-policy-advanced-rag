...
| T-5.3 | RAGAS: Context Precision, Context Recall, Faithfulness, Answer Correctness | T-5.2 | `WIP` (3rd real bug found+fixed 2026-09-04: installed ragas 0.3.1 -- older than the 0.4.3 dev-reference venv, not newer as first assumed -- calls `nest_asyncio.apply()` unconditionally with no opt-out; fixed by neutralizing `nest_asyncio.apply()` before importing ragas, matching 0.4.3's own guarded behavior -- awaiting re-run) |
