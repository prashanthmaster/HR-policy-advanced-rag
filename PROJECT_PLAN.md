...
| **M6** | Freshness demoable | A document edited in Drive is picked up, re-indexed incrementally, and the answer changes correctly on camera | D5–D6 | `DONE` -- T-6.7's live demo run twice for real 2026-09-04/05, see Phase 6 detail and Change Log |
| **M7** | Regression-gated | CI runs the eval on push and fails the build on regression from recorded baseline | D6 | `DONE` -- real GitHub Actions run passes on good code and fails on a deliberately-introduced regression, both proven live 2026-09-05, see Phase 7 detail and Change Log |
...
| T-6.2 | Upload corpus to a Drive folder as the live source of record | T-6.1 | `DONE` |
| T-6.3 | Change detection by polling | T-6.2 | `DONE` |
| T-6.4 | Incremental re-index of only the changed document | T-6.3, M2 | `DONE` |
| T-6.5 | Version-event handling on `SUBSTANTIVE` / `SUNSET` — supersede predecessor at clause level | T-6.4 | `DONE` |
| T-6.6 | Faithfulness reused live as refusal gate; calibrate the threshold **against the over-refusal counter**, not in isolation | M5 | `DONE` |
| T-6.7 | Scripted live demo: edit a clause in Drive → show pickup → show the answer change and the supersession flag | T-6.5 | `DONE` |
| T-6.8 | Re-run eval post-freshness; confirm no regression | T-6.6 | `DONE` |
| T-6.9 | Golden-set impact flagging: cross-reference a changed clause_id against every golden probe's `expected_clause_ids`, flag any overlap on re-index | T-6.5 | `DONE` |
...
### Phase 7 — CI gate · `DONE` · D6

| ID | Task | Depends on | Status |
|---|---|---|---|
| T-7.1 | GitHub Actions workflow running the eval harness | M5 | `DONE` |
| T-7.2 | Gate on **no regression from recorded baseline** — not on invented absolute thresholds | T-7.1 | `DONE` |
| T-7.3 | Publish per-run scorecard as a build artifact | T-7.2 | `DONE` |

**Exit criterion (M7): MET.** A deliberately-introduced regression (commit `c3dae28`, run 33955216704) failed the build for real -- see Change Log.
...
| 2026-09-04 | T-6.8 | Four-class confusion matrix, **re-run post-freshness (Session 10)**, after live Phase 6 edits to India gratuity eligibility and UAE gratuity/leave clauses | Class match **18/24** (up from 14/24 -- no regression, a real improvement, most likely just corpus/index drift since Session 8 rather than anything Phase 6 caused); over-refusal still **1**, still the **same P-01**, still the **same already-documented reranker-ordering limitation** -- Phase 6's live edits did not introduce any new over-refusal | `build/confusion_matrix_result.json`, `eval/run_confusion_matrix.py` |
...
| Date | Change |
|---|---|
| 2026-09-05 | **M7 exit criterion met: a deliberately-introduced regression really does fail the build.** Temporarily bumped the calibrated `DEFAULT_MIN_RERANK_SCORE` (0.001, chosen from real data in Session 6) to 0.5 in `retrieval/hybrid_search.py` (commit `c3dae28`), which starves the pipeline of retrieved context by pushing nearly every real candidate below the floor. Pushed and watched CI run 33955216704 fail for exactly the right reason: `confusion_matrix.class_match_count` collapsed from 18/24 to 13/24 and `over_refusal_count` tripled from 1 to 3 (both held at zero tolerance), `citation_accuracy.items_missing_expected_citation` rose from 5 to 8, and even the judge-scored `ragas.mean_context_precision` cratered from 0.7383 to 0.5278 -- far past every tolerance, on the metric this project's whole pitch rests on. Reverted the constant back to 0.001 immediately after confirming the failure. **Phase 7 (M7) is now fully DONE**: T-7.1-T-7.3 all complete, both halves of the exit criterion proven for real (a good change passes, a bad change fails), not just asserted. |
| 2026-09-05 | **T-7.1/T-7.2/T-7.3 built and confirmed with a real, passing GitHub Actions run (run 33953974505) -- five real failures found and fixed along the way, none of them hidden.** Built `.github/workflows/ci.yml` (unit tests -> real vector-index build with live OpenAI embeddings -> the three eval scripts -> `scripts/check_regression.py` gates on regression vs. `eval/baseline/regression_baseline.json` -> scorecard published as a build artifact). Scope decision made explicitly with Prashanth first: CI never touches Google Drive at all -- the freshness/live-sync feature (T-6.1-T-6.9) stays proven by the existing manual live demo instead, since automating it in CI would need the OAuth token as a weekly-refreshed secret (Google's "Testing" publishing status expires refresh tokens every 7 days), adding a failure mode unrelated to code quality. The first real run failed five times in a row, each a genuine, different, previously invisible bug, fixed in sequence rather than worked around: **(1)** `httpx.LocalProtocolError: Illegal header value` on the very first OpenAI call -- none of the 7 scripts reading `OPENAI_API_KEY` ever stripped it, so a pasted-in GitHub secret with a trailing newline broke the Authorization header outright; fixed by stripping the value right after each script's existing not-set guard. **(2)** `No module named 'langchain_community.chat_models.vertexai'` -- traced to the real installed package on Prashanth's machine and found it was never a real langchain-community file at all: it was a 5-line hand-written compatibility shim (see the 2026-09-04 change log entry on the first Session 6 vertexai fix) dropped straight into `.venv-win`'s site-packages to work around `ragas` 0.3.1's eager top-level `ChatVertexAI` import -- invisible to git, so it silently vanished on CI's fresh install and reproduced the exact same crash there. Fixed properly this time: the same stub, registered as a fake module in `sys.modules` right before `ragas` is imported, now lives in `eval/run_ragas_eval.py` itself and travels with the code on any machine. **(3)** Pinned `langchain`/`langchain-community`/`ragas`/`openai`/`datasets` to the exact versions already proven to work locally (captured via `importlib.metadata`, since `pip` itself was missing as a module in `.venv-win`), after the first fix surfaced version drift between a fresh Linux install and Prashanth's machine as a contributing risk. **(4)** Once the pipeline finally ran end to end, `check_regression.py` correctly failed the build on a real difference (Context Precision 0.7383 vs. the Windows-recorded baseline's 0.7707) -- root cause was an honest environment mismatch, not a code bug: the recorded baseline used a fully warm local embedding cache, while GitHub Actions never caches embeddings between runs. Fixed by re-baselining `eval/baseline/regression_baseline.json` from a real CI run's own scorecard artifact instead of the local one, since every future comparison is CI-vs-CI anyway. **(5)** The very next CI run -- identical code, identical corpus -- failed again, this time on three different metrics, swinging up to ~4.5% from the just-updated baseline: real, measured evidence that gpt-4o-mini's judge calls plus CI's lack of embedding caching produce more run-to-run noise (~4.5%) than the original 0.03 tolerance (calibrated from only two local runs that happened to land within 0.002 of each other) actually allowed for. Widened `TOLERANCE_SCORE` to 0.08 -- the measured spread plus a safety margin -- for the four RAGAS metrics and citation accuracy's mean, while deliberately leaving the confusion-matrix counts and citation-accuracy problem counts at **zero tolerance**, since those have stayed bit-identical across every real run (local and CI) and are this project's primary "does it make the right decision" metric. None of this changes the actual recorded quality numbers (Faithfulness ~0.47-0.51, Answer Correctness ~0.35-0.38, Citation Accuracy 0.69, confusion matrix 18/24 with the same single P-01 over-refusal) -- every fix here was about making the measurement mechanism itself trustworthy, not about the scores it measures. **M7's exit criterion is only half met**: a real CI run now passes end to end, but a deliberately-introduced regression actually failing the build has not yet been demonstrated -- that's the next step. |
| 2026-09-05 | **T-6.9 built and confirmed live: golden-set impact flagging closes the eval-staleness gap found in T-6.8.** Session 10 flagged a real hole: T-6.5's version-event detector tells a human when a corpus clause needs review, but says nothing about whether that clause is the answer key for a golden probe -- so a golden_answer can silently go stale. Fix: `drive_sync/golden_impact.py`'s `find_affected_probes()`, a small pure function (no API calls, no new external dependency) that intersects a re-index's changed clause_ids against every golden probe's existing `expected_clause_ids` field -- both pieces of data already existed, they just weren't cross-referenced. Wired into `scripts/reindex_from_drive.py`'s existing review-notice output. 7 new tests (`tests/test_golden_impact.py`), including one against the real 24-item golden set. Confirmed live end-to-end: Prashanth reverted the UAE gratuity and annual-leave clauses in Drive back to their real Federal Decree-Law 33/2021 values (undoing Session 10's earlier demo edits, which had left the corpus with fictional numbers and two stale golden answers -- P-03, P-3b). The re-index correctly re-classified both as `SUBSTANTIVE` and the new flagging correctly named exactly `P-03` and `P-3b` and no others -- a real, live proof, not just a unit test. Since the corpus is now back to the real statutory values, both golden answers are correct again and needed no editing. Full suite: **196/196 passing** (189 baseline + 7 new). This closes the loop identified in T-6.8: the freshness pipeline (T-6.1-T-6.9) now flags corpus drift AND eval-ground-truth drift, not just the former. |
| 2026-09-04 | **T-6.8 DONE: post-freshness eval re-run shows no regression -- and surfaces a real, undecided finding about stale golden answers.** Re-ran `eval/run_confusion_matrix.py` after all of Phase 6's live edits. Result: class match **18/24**, up from the Session 8 baseline of 14/24; over-refusal count unchanged at **1**, and it's the same P-01 item with the same already-documented root cause (reranker ordering) -- none of this session's live Drive edits (India gratuity eligibility 5->4 years, UAE gratuity eligibility 1->3 years / first-tier rate 21->25 days, UAE annual leave 30->27->28 calendar days) introduced a new pipeline regression. Separately, and NOT fixed here per explicit direction: reading the golden set against the live corpus confirms two probes' recorded golden_answer text is now stale relative to the live index -- P-03 (UAE, 7 years' service) still states "21 days' basic wage" for the first tier when the live clause now says 25; P-3b (UAE annual leave, Dubai) still states "30 CALENDAR days" when the live clause now says 28. The confusion matrix doesn't catch this (it only checks ANSWERED/NEEDS_CLARIFICATION/INSUFFICIENT status, not answer content), so it would only show up in a RAGAS Answer Correctness or Citation Accuracy re-run -- deliberately not run, since scoring against known-stale ground truth would produce a misleading number, and chasing it further doesn't serve the stated goal (a defensible resume project, not a fully-maintained one). This is left on record as the honest, interesting story: a live-freshness pipeline that updates the index correctly can still leave its own eval's ground truth behind, and that gap is a real, known, deliberately-unaddressed limitation, not something papered over. **Phase 6 (M6) is now fully DONE: T-6.1 through T-6.8 all complete, all proven live on Prashanth's own machine with real Drive edits and real API calls, not just unit tests.** |
| 2026-09-04 | **T-6.7 DONE: live demo run end-to-end, real Drive edit -> visible answer change, confirmed twice.** First real run used a UAE gratuity question with an edit to an unrelated clause (annual leave) -- pipeline correctly detected/reindexed/classified it SUBSTANTIVE, but the gratuity answer correctly did NOT change, since retrieval correctly judged the leave clause irrelevant to a gratuity question (precision working as intended, not a bug) -- an honest finding worth keeping in the record rather than only showing the clean run. Second run asked about UAE annual leave instead: BEFORE already reflected the prior edit (27/3 days) confirming the earlier reindex had stuck; edited `UAE-DL33-ART29-LEAVE` again live in Drive (27->28 calendar days, 3->4 days/month), correctly classified SUBSTANTIVE with the human-review note, and the AFTER answer visibly flipped to 28/4 days -- RESULT: "the answer CHANGED after the live edit." Faithfulness gate passed both times (0.597 and 0.775 against the 0.30 threshold), confirming the gate doesn't false-positive on a legitimately correct, freshly-updated answer. T-6.7 -> `DONE`, full Phase 6 mechanism (T-6.1-T-6.7) now proven live end-to-end with zero remaining WIP items before T-6.8. Next: T-6.8 -- re-run Phase 5 eval scripts post-freshness and check honestly whether this session's live edits (India gratuity eligibility, UAE gratuity formula, UAE annual leave) have made any golden probe's recorded golden_answer stale. |
| 2026-09-04 | **T-6.6 DONE: threshold calibrated from real data (0.30); T-6.7 built (thin demo orchestration script).** Real calibration run (`scripts/calibrate_faithfulness_gate.py`, 13 ANSWERED golden items scored) found even correct answers score faithfulness 0.333-0.737 -- consistent with Phase 5's own measured baseline mean (0.497) -- because this generator is deterministic/citation-grounded rather than free-form LLM prose, which RAGAS's claim-decomposition metric wasn't built assuming. Any threshold >= 0.40 already wrongly refuses real correct answers (2 at 0.40, a majority at 0.50+) -- Risk RK-2's exact failure mode, confirmed in real numbers. Set `DEFAULT_FAITHFULNESS_THRESHOLD = 0.30`: refuses zero of the 13 real items (no regression risk) while remaining a real trip-wire against a genuinely degenerate future answer, not a claim that 0.30 is a good abstract bar. Full reasoning and per-item numbers recorded here and in `build/faithfulness_calibration_result.json` rather than silently picked. T-6.7: added `VectorIndex.close()` (releases the on-disk Qdrant lock -- needed since T-6.4's `reindex_source_file()` opens its own client against the same path) and `scripts/live_demo.py`, a thin 5-step orchestration script (ask -> pause for a live Drive edit -> poll/re-index -> ask again -> show the faithfulness-gate verdict) containing NO new pipeline logic of its own, by design -- it only calls T-6.1-T-6.6's already-proven pieces. 189/189 tests passing (unchanged; `close()` and the threshold value needed no new tests beyond the existing gate-logic suite). Not yet run for real end-to-end -- needs Prashanth's terminal. T-6.7 `WIP` pending that run; T-6.8 (post-freshness eval re-run) is next, and should honestly check whether Session 10's own live demo edits (India gratuity eligibility, UAE gratuity formula) changed the underlying content behind golden probes P-01/others in a way that makes their recorded golden_answer stale -- a real, interesting case of the freshness feature affecting the eval's own ground truth, not something to smooth over if it shows up. |
| 2026-09-04 | **T-6.6 built (Session 10 continued): live faithfulness refusal gate, not yet calibrated.** `grading/ragas_client.py` (`score_faithfulness()`) factors out a minimal single-sample RAGAS Faithfulness call, deliberately NOT a refactor of the already-DONE, already-proven `eval/run_ragas_eval.py` -- the tricky nest_asyncio guard is duplicated (small, well-understood, cross-referenced) rather than risking a working, measured script for DRY's sake. `grading/faithfulness_gate.py` (`apply_faithfulness_gate()`): takes an ANSWERED `PipelineResult`, scores it live against its own cited context, and converts it to `INSUFFICIENT` (a real-time refusal) if the score is below a threshold -- deliberately NOT wired into `answer_query()` itself (avoids a circular import, and keeps gating an explicit caller decision, not a silent pipeline default). `DEFAULT_FAITHFULNESS_THRESHOLD` stays `None` (no-op) until real calibration data exists -- same discipline as Session 6's `min_rerank_score` (Convention 14): never invent an unmeasured constant. `PipelineResult` gained `faithfulness_score`/`refused_by_faithfulness_gate` fields so a refusal carries the score that caused it. `scripts/calibrate_faithfulness_gate.py` written for the real run: scores every ANSWERED golden item's real live faithfulness, sweeps candidate thresholds (0.3-0.9), and reports the over-refusal cost at each -- explicitly documented as measuring only the false-positive (over-refusal) side, since this corpus's citation-grounded generator has no confirmed-unfaithful golden case to check the true-positive catch rate against (an honest limitation, not glossed over). 6 new tests (`tests/test_faithfulness_gate.py`) using a fake injected scorer -- confirms no-op when unthresholded, correct pass-through/refusal at the boundary, no mutation of the original result, and non-ANSWERED statuses are untouched. 189/189 tests passing (183 + 6 new). Not yet run for real -- needs Prashanth's terminal for the real calibration sweep before a threshold is actually set. T-6.6 `WIP` pending that. |
| 2026-09-04 | **T-6.5 DONE: version-event flagging confirmed live.** Prashanth edited "Tier 1 — UAE Law" for real -- the gratuity eligibility threshold (one year -> three years) and the first-tier rate (21 -> 25 days' wage) in `UAE-DL33-ART51-GRATUITY-FORMULA`. `scripts/reindex_from_drive.py` correctly re-indexed the change (1 real embedding call, 5 served from cache) and `classify_change()` correctly classified it `SUBSTANTIVE`, printing the human-review note with no automatic version metadata written -- exactly the Option B contract Prashanth chose. T-6.5 -> `DONE`. All of T-6.1-T-6.5 now proven live with real edits (India gratuity eligibility, UAE gratuity formula), not just unit tests. Remaining Phase 6 work: T-6.6 (faithfulness as a live refusal gate, calibrated against the over-refusal counter), T-6.7 (scripted live demo), T-6.8 (post-freshness eval re-run, confirm no regression). |
| 2026-09-04 | **T-6.5 built (Session 10 continued): clause-level version-event detection, Option B (flag, never auto-decide).** Found `ingestion/change_kind.py` (T-2.5, Phase 2) already existed and was already unit-tested but never actually called by anything -- classifies a clause's old-vs-new body text as NO_OP/EDITORIAL/SUBSTANTIVE/ADDITION/SUNSET. Presented Prashanth the real design choice this surfaces (auto-generate version metadata by pattern-matching effective dates out of free text, vs. detect and flag for human confirmation) -- **Prashanth chose Option B**, consistent with this project's standing rule against inventing unverified facts. Built `drive_sync/version_events.py` (`detect_version_events()`, wired into `drive_sync/reindex.py` by capturing the OLD corpus file's clauses before it's overwritten with the new Drive text): every clause_id present in both old and new versions gets classified; SUBSTANTIVE/SUNSET/ADDITION are surfaced as `VersionEvent`s needing human review (with an explanatory note), NO_OP/EDITORIAL are silently skipped. The module never writes supersedes/superseded_by/effective_date itself -- retrieval is still always updated to match the live document (T-6.4's job, unaffected), but the corpus's own version lineage is only ever changed by a human, same as every existing versioned clause pair in this corpus. `scripts/reindex_from_drive.py` now prints a clear review list after re-indexing. 7 new tests (`tests/test_version_events.py`) confirm: no event on NO_OP/EDITORIAL; SUBSTANTIVE/SUNSET/ADDITION all flagged with a real note; multi-document clause-id matching is correct; and an explicit guardrail test that the returned VersionEvent never carries a supersedes/superseded_by/effective_date field (the Option B contract, asserted, not just described). 183/183 tests passing (176 + 7 new). Not yet run for real against a live Drive edit -- T-6.4's test edit was already marked seen, so a fresh test edit is needed to see this fire live; T-6.5 `WIP` pending that. |
| 2026-09-04 | **T-6.4 DONE: incremental re-index confirmed live end to end.** Prashanth ran the bridging `--live` rebuild first (all 84 units served from the embedding cache, zero real API calls -- confirms the source_file payload backfill was free) then `scripts/reindex_from_drive.py` against the real India-law edit sitting in Drive since T-6.3: detected the 1 changed file, re-parsed it (8 clauses), embedded exactly 2 texts via a real OpenAI call (the two clauses whose text actually changed) with the other 6 served from cache, and `VectorIndex.reindex_source_file()` removed the file's old points and upserted the 8 new ones -- the other 7 documents' points were never touched. Real, measured proof of the whole Phase 6 mechanism: an edit in Drive costs one real embedding call for the genuinely-changed content and zero disruption to everything else. T-6.4 -> `DONE`. Remaining Phase 6 work: T-6.5 (change-kind classification -- EDITORIAL vs SUBSTANTIVE vs SUNSET -- and clause-level supersession, noting `ingestion/change_kind.py` already exists and needs checking before building anything new per Convention 12), T-6.6 (faithfulness refusal gate), T-6.7 (scripted live demo), T-6.8 (post-freshness eval re-run). |
| 2026-09-04 | **T-6.4 built (Session 10 continued): incremental vector re-index, deliberately full BM25 rebuild.** Found `VectorIndex.build()` always deleted/recreated the whole Qdrant collection -- no incremental path existed. Added `source_file` to every point's payload and a new `VectorIndex.reindex_source_file()`: deletes only the existing points whose `source_file` matches the changed document (via a Qdrant `Filter`/`FieldCondition`, so a clause *removed* from the source doesn't linger as a stale point, not just additions/edits), then embeds and upserts the freshly-parsed units for that file only -- every other document's points untouched. `drive_sync/reindex.py` wires this to Drive: export live text -> overwrite the local corpus file -> re-parse just that file -> `reindex_source_file()` -> `mark_seen()` only on success (a failed re-index is retried on the next poll, not silently dropped). Deliberately did NOT build an incremental BM25 path -- `BM25Okapi` computes corpus-wide document-frequency stats at construction time, so "update just one document" isn't a meaningful operation for it, and every real script in this project already rebuilds `BM25Index` from a fresh `parse_corpus()` call each run (free, no API cost) rather than loading a persisted one -- since step 1 already overwrote the local corpus file, BM25 picks up the edit for free with no separate step. This asymmetry (vector: incremental + persisted; BM25: full rebuild, cheap, never persisted) is a deliberate scope call, not an oversight. Added 2 new tests (`test_reindex_source_file_replaces_only_that_files_points`, `test_reindex_source_file_without_a_prior_build_raises`) using `MockEmbedder` -- confirms other documents' points survive untouched, a removed clause's stale point is gone, and edited content is retrievable by its new text, not the old point re-upserted verbatim. 176/176 tests passing (174 baseline + 2 new). Not yet run for real against the live Drive edit already sitting in India Law (Session 10's T-6.3 test edit) -- needs Prashanth's terminal, and one bridging full `--live` rebuild first so every existing point picks up the new `source_file` payload field (cheap: the embedding cache means no clause gets re-billed). T-6.4 `WIP` pending that real run. |
| 2026-09-04 | **T-6.3 DONE: change detection confirmed live with a real, substantive edit.** Prashanth opened "Tier 1 — India Law" directly in Google Docs (via its `webViewLink`) and appended a real clause change -- the gratuity eligibility period reduced from five to four years, effective 1 April 2026 (a genuinely useful test edit: a real `SUBSTANTIVE`-with-effective-date change, not a cosmetic one, which will matter for T-6.5's change-kind classification). `scripts/check_for_drive_changes.py` (no `--commit`) correctly reported exactly that one file as `MODIFIED` (old modifiedTime 15:24:14, new 15:29:18) and left the other 7 untouched. T-6.3 -> `DONE`. T-6.4 (incremental re-index of only the changed document) is next -- this same live-edited document is the natural real test case for it. |
| 2026-09-04 | **T-6.2 DONE, T-6.3 built (Session 10 continued).** `scripts/upload_corpus_to_drive.py` ran for real from `.venv-win` (needed one more one-time OAuth consent after the drive.readonly -> drive.file scope widen): all 8 corpus files -- India/UAE/UAE-DIFC/Germany law + the 4 Meridian policy docs -- uploaded as native Google Docs into the Drive folder, `drive_sync/corpus_manifest.json` written mapping each local corpus path to its Drive file id. T-6.2 -> `DONE`. Built T-6.3 (`drive_sync/change_detector.py`): polling, not Drive's push-notification API -- this project has no public webhook endpoint, and a poll is simpler to run/demo/reason about at this scale (deliberate scope call, not an oversight). Compares each manifest file's live `modifiedTime` against `drive_sync/sync_state.json`; pure detection only -- never writes state itself, so a detected change can't be silently lost if T-6.4's re-index fails partway through (`mark_seen()` is a separate call, meant to be made only after a successful re-index). `scripts/check_for_drive_changes.py` written as a manual smoke test (`--commit` flag to establish the first baseline without the initial upload itself reading as 8 false-positive changes). Not yet run for real -- needs Prashanth's terminal. T-6.3 `WIP` pending that run and T-6.4 actually consuming its output. |
