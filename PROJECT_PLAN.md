...
**Exit criterion (M7):** a deliberately-introduced regression fails the build. **Not yet demonstrated** — a real, passing CI run exists (see Change Log), but the other half of the exit criterion (proving the gate actually catches a bad change) is still open.
